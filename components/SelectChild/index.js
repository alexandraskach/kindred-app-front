        ...
        <div className="SelectChild__current__informations">
          { currentChild && (
            <div className="d-flex align-items-center">
              <div className="Picture Picture--letter mr-1">{currentChild.firstName.split('')[0]}</div>
              <span>
          ...
          <div className="SelectChild__others__container">
            { children.map((child) => {
              return child.id != currentChild?.id ? (
                <div onClick={(e) => handleClick(child.id, router, e.target)} key={child.id} className="SelectChild__others__container__item">
